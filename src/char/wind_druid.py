...
class Wind_Druid(IChar):
    # def __init__(self, skill_hotkeys: dict, pather: Pather):
    #     Logger.info("Setting up Wind Druid Character")
    ...
    def __init__(self, skill_hotkeys: dict, pather: Pather):
        # os.system('color')
        Logger.info("\033[94m<<Setting up Wind_Druid>>\033[0m")
        super().__init__(skill_hotkeys)
        self._pather = pather
        # self._do_pre_move = True
        #custom necro pathing for pindle
        self._pather.adapt_path((Location.A5_PINDLE_START, Location.A5_PINDLE_SAFE_DIST), [100,101,102,103])
        self._pather.adapt_path((Location.A5_PINDLE_SAFE_DIST, Location.A5_PINDLE_END), [104])
        self._pather.offset_node(101, [100,-15])
        self._pather.offset_node(102, [15, 0])
        self._pather.offset_node(103, [50, -75])
        self._pather.offset_node(104, [-100, -100])
        # custom necro pathing for eldritch
        self._pather.adapt_path((Location.A5_ELDRITCH_START, Location.A5_ELDRITCH_SAFE_DIST), [120, 121,122])
        self._pather.adapt_path((Location.A5_ELDRITCH_SAFE_DIST, Location.A5_ELDRITCH_END), [123])
        self._pather.offset_node(120, [0, 50])
        self._pather.offset_node(121, [0, 0])
        self._pather.offset_node(122, [0, 0])
        self._pather.offset_node(123, [-100, -100])
    ...
    def pre_buff(self):
        ...
        #     self._pre_buff_cta()
        if self._skill_hotkeys["cyclone_armor"]:
        ...
        if self._skill_hotkeys["oak_sage"]:
            ...
            mouse.click(button="right")
            wait(self._cast_duration)
        if self._skill_hotkeys["spirit_wolf"]:
            keyboard.send(self._skill_hotkeys["spirit_wolf"])
            wait(0.1, 0.13)
            for _ in range(5):
                mouse.click(button="right")
                wait(.5)
        if self._skill_hotkeys["dire_wolf"]:
            keyboard.send(self._skill_hotkeys["dire_wolf"])
            wait(0.1, 0.13)
            for _ in range(3):
                mouse.click(button="right")
                wait(.5)
        # if self._skill_hotkeys["grizzly"]:
        #     keyboard.send(self._skill_hotkeys["grizzly"])
        #     wait(0.1, 0.13)
        #     mouse.click(button="right")
        #     wait(self._cast_duration)
        # if self._skill_hotkeys["hurricane"]:
